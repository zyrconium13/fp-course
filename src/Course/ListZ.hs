...
lengthAcc :: List a -> Int -> Int
lengthAcc Nil n = n
lengthAcc (_ :. ys) n = lengthAcc ys (n + 1)
...
(++) ::
  ...
  List a
(++) Nil ys = ys
(++) (x :. xs) ys = x :. (xs ++ ys) -- parentheses are redundant but make code more explicit
